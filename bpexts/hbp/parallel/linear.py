...
class HBPParallelLinear(HBPParallel):
    ...
    def __init__(self, layer, max_blocks):
        ...
        if not layer.__class__ == self.contained_class:
            raise ValueError(
                "Expecting layer of type {}, got {}".format(
    ...
    def _create_main_parameters(self):
        ...
        del self.main.bias
        self.main.bias = (
            None if not has_bias else cat([c.bias for c in self.parallel_children()])
        )
    ...
    def _create_parallel_children(self, layer):
        ...
        children = {}
        for idx, (out, w, b) in enumerate(zip(out_features, w_chunks, b_chunks)):
    ...
    def compute_backward_hessian_quantities(self):
        ...
        if self.main.average_param_jac is True:
            self._reference_mean_input()
        elif self.main.average_param_jac is False:
    ...
    def parameter_hessian(self, output_hessian):
        ...
        out_h_split = [
            (output_hessian.chunk(self.max_blocks, 0)[i]).chunk(self.max_blocks, 1)[i]
