...
class HBPParallelSequential(HBPSequential):
    ...
    def __init__(self, num_blocks, *layers):
        ...
        converted = [self._convert_to_parallel(layer, num_blocks) for layer in layers]
    ...
    def _convert_to_parallel(self, layer, num_blocks):
        ...
        if target_cls is None:
            warn(
                "Conversion to parallel: The following layer will be"
                "left unchanged:r\n{}".format(layer)
            )
            return layer
        else:
    ...
    def _find_target_class(cls, layer_cls):
        ...
        subcls = [
            cls.conversions[sub]
            for sub in cls.conversions.keys()
        ...
        if len(subcls) == 1:
            ...
            raise ValueError(
                "Found multiple target classes to convert"
                "{} to: {}\nSupported conversions:\n{}".format(
                    layer_cls, subcls, cls._supported_conversions()
                )
            )
        else:
            raise ValueError(
                "No conversion strategy found. Supported"
