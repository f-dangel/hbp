...
def cg(A, b, x0=None, tol=1e-5, maxiter=None, atol=None):
    ...
    TODO: It is possible for the current implementation to get stuck
          if CG does not converge, since the maxiters cannot be
    ...
    atol = torch.tensor([0.0 if atol is None else atol], device=device)
    ...
    # stopping criterion
    norm_bound = torch.tensor(
        [tol * torch.norm(b), atol], device=device, dtype=rs_old.dtype
    )
