...
from bpexts.utils import exact_hessian, exact_hessian_diagonal_blocks
...
def involved_function_with_parameters():
    ...
    # parameters
    x = tensor([[1.0, -2], [3, -4]], requires_grad=True)
    y = tensor([5.0, 6], requires_grad=True)
    parameters = [x, y]
    # input (no parameter)
    input = tensor([1.0, 2])
    # composition of functions
    temp = x.matmul(input) + y
    temp_result = tensor([2.0, 1])
...
def test_exact_hessian_simple():
    ...
    # expected outcome
    result = tensor([[1.0, 4, 5], [4, 2, 6], [5, 6, 3]])
    # compute and compare results
    hessian = exact_hessian(f, parameters)
...
def test_exact_hessian_diagonal_blocks_involved():
    ...
    f, parameters = involved_function_with_parameters()
    # expected outcome
    block1 = tensor([[2.0, 4, 0, 0], [4, 8, 0, 0], [0, 0, 2, 4], [0, 0, 4, 8]])
    block2 = tensor([[2.0, 0], [0, 2]])
    results = [block1, block2]
    # compute and compare
    hessian_blocks = exact_hessian_diagonal_blocks(f, parameters)
    for block, result in zip(hessian_blocks, results):
