...
def torch_fn():
    ...
    return MaxPool2d(kernel_size, padding=padding, dilation=dilation, stride=stride)


def cvp_fn():
    """Create a 2d max pool layer with CVP functionality."""
    return CVPMaxPool2d(kernel_size, padding=padding, dilation=dilation, stride=stride)
...
def cvp_from_torch_fn():
    """Create CVPMaxPool2d from torch layer."""
    torch_layer = torch_fn()
    return CVPMaxPool2d.from_torch(torch_layer)


for name, test_cls in set_up_cvp_tests(
    torch_fn,
    cvp_from_torch_fn,
    "CVPMaxPool2dFromTorch",
    input_size=input_size,
    atol=atol,
    rtol=rtol,
    num_hvp=num_hvp,
