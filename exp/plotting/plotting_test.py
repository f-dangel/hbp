...
from ..utils import directory_in_data, directory_in_fig
...
filenames = [
    ...
    for f in ["dummy_A.csv", "dummy_B.csv", "dummy_C.csv", "dummy_D.csv"]
]
# labels in plot
...
def test_plotting():
    ...
    plt.figure()
    # set color cycle
    matplotlib.rcParams["axes.prop_cycle"] = matplotlib.cycler(color=Blues_4.mpl_colors)
    xlabel = r"$x$"
    ylabel = r"$f(x)$"
    OptimizationPlot.create_standard_plot(xlabel, ylabel, filenames, labels)
    plt.legend()
